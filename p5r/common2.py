...
def create_graph(batch_size,
    ...
    with graph.as_default():
        ...
        print(layers)
        connected_size = img_width * img_height
        shrink = 1
        for i in range(len(layers) - 1):
            ...
            print(connected_size)
            if use_max_pool:
                # Our fully connected size decreases by a factor of 4 for each layer
                shrink = shrink * (i+1)

        print(connected_size)
        connected_size = img_width * img_height * layers[-1] / shrink
        print(connected_size)
